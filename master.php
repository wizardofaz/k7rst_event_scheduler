...
function list_events_from_master_with_status() {
    ...
    log_msg(DEBUG_INFO, "ℹ️ Retrieved " . count($events) . " event(s) with status.");
...
function get_event_creation_sql_from_master($event_name) {
    ...
    if ($stmt->fetch()) {
        $stmt->close();
        $conn->close();
        log_msg(DEBUG_INFO, "ℹ️ Fetched SQL for event '$event_name': $sql");
        return $sql;
    } 

    log_msg(DEBUG_INFO, "ℹ️ Could not fetch SQL for event '$event_name': " . ($sql ?? '[undefined]') . "; will fetch default.");
    $stmt->close();
...
function create_event_db_tables($event_name) {
    ...
    if (!$sql) {
        log_msg(DEBUG_ERROR, "❌ create_event_db_tables: Failed to read SQL template for $event_name from master db.");
        return false;
    }

    $connect_info = get_event_connection_info_from_master($event_name);
    if (!$connect_info) {
        log_msg(DEBUG_ERROR, "❌ create_event_db_tables: Failed to get db info for $event_name.");
        return false;
    }

    $conn = new mysqli($connect_info['host'], $connect_info['user'], $connect_info['pass'], $connect_info['name'] );
    if ($conn->connect_error) {
        log_msg(DEBUG_ERROR, "❌ create_event_db_tables: Failed to get db connection $event_name, " . $conn->connect_error . ".");
        return false;
    }
    ...
    foreach ($statements as $statement) {
        ...
        if (!$conn->query($statement)) {
            log_msg(DEBUG_ERROR, "❌ create_event_db_tables: SQL error: " . $conn->error . "\n\tStatement: $statement.");
            $conn->close();
            return false;
        } else {
            log_msg(DEBUG_INFO, "✅ create_event_db_tables: SQL good query: \n\tStatement: $statement.");
        }
    }

    log_msg(DEBUG_INFO, "✅ create_event_db_tables: successfully created all tables for $event_name.");
    $conn->close();
    return true;
}
...
function get_event_db_connection_from_master($event_name) {
    ...
    if (!$conn_info) {
        log_msg(DEBUG_ERROR, "❌ master: no connection info found for event '$event_name'");
        return null;
    }

    $conn = new mysqli($conn_info['host'], $conn_info['user'], $conn_info['pass'], $conn_info['name']);
    if (!$conn->connect_error) return $conn;
    log_msg(DEBUG_ERROR, "❌ master: could not connect to event db $event_name, SQL error: " . $conn->connect_error);
    return null;
}